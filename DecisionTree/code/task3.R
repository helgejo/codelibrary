...
# With no discretization Accuracy : 0.8628
...
#train the model
model <- train(V15 ~., data = train.train, method = 'gbm', trControl=myControl)
...
write.table(indx, "output/task3.out", row.names=FALSE, col.names = TRUE , quote=FALSE, sep = ",")
#importance(model)
#varImpPlot(model)
